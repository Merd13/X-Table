...
Now you can rename `src/Table.css` to `src/Table.scss` and run `npm run watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/Table.css`. Since `src/Table.js` still imports `src/Table.css`, the styles become a part of your application. You can now edit `src/Table.scss`, and `src/Table.css` will be regenerated.

To share variables between Sass files, you can use Sass imports. For example, `src/Table.scss` and other component style files could include `@import "./shared.scss";` with variable definitions.
...
Import required React Bootstrap components within ```src/Table.js``` file or your custom component files:

```js
import { Navbar, Jumbotron, Button } from 'react-bootstrap';
```

Now you are ready to use the imported React Bootstrap components within your component hierarchy defined in the render method. Here is an example [`Table.js`](https://gist.githubusercontent.com/gaearon/85d8c067f6af1e56277c82d19fd4da7b/raw/6158dd991b67284e9fc8d70b9d973efe87659d72/App.js) redone using React Bootstrap.
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `Table.test.js` and `Table.js` are in the same folder, the test just needs to `import Table from './Table'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
Unlike the previous smoke test using `ReactDOM.render()`, this test only renders `<Table>` and doesn’t go deeper. For example, even if `<Table>` itself renders a `<Button>` that throws, this test will pass. Shallow rendering is great for isolated unit tests, but you may still want to create some full rendering tests to ensure the components integrate correctly. Enzyme supports [full rendering with `mount()`](http://airbnb.io/enzyme/docs/api/mount.html), and you can also use it for testing state changes and component lifecycle.
